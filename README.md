...
If you won't use GNU Make, Docker commands will have to be executed from within the `./platform/nginx-java-17/docker` and `./platform/postgres-16.4/docker` directories, e.g.:
```bash
./platform/nginx-java-17/docker $ sudo docker compose up --build --no-recreate -d
```
...
> **Note**: There is a `./platform/nginx-java-17/docker/.env.example` file with the variables required to build the container by `docker-compose.yml` file to create the container. Otherwise, if no GNU Make is available on developer's machine, it is required to create its `.env` manually to build the container.

API environment file content at `./platform/nginx-java-17/docker`:
```bash
COMPOSE_PROJECT_LEAD=javasb
COMPOSE_PROJECT_CNET=jar-dev
COMPOSE_PROJECT_IMGK=alpine3.22-nginx1.28-java17
COMPOSE_PROJECT_NAME=jar-apirest-dev
...
COMPOSE_PROJECT_USER=myuser
COMPOSE_PROJECT_GROUP=myuser
```
<br>
...
PROJECT_CNET=jar-dev

APIREST_PLTF=nginx-java-17
APIREST_IMGK=alpine3.22-nginx1.28-java17
APIREST_PORT=4501
APIREST_BIND="../../../apirest"
APIREST_CAAS=jar-apirest-dev
APIREST_CAAS_USER=myuser
APIREST_CAAS_GROUP=myuser
APIREST_CAAS_MEM=512M
APIREST_CAAS_SWAP=1G
...
DATABASE_PORT=4500
DATABASE_CAAS=jar-pgsql-dev
DATABASE_CAAS_MEM=512M
DATABASE_CAAS_SWAP=1G
...
<span color="orange"><b>IMPORTANT:</b></span> Once the container is built and running, the Nginx server block serves at port 80 proxing to port 8080 to be handled by Java. Though the container is ready to used, first installation will fail as it is needing an application inside the container.
...
To preview the successful installation on browser, there is a basic **SpringBoot3** sample at `./resources/docs/apirest/default-install/`. Copy its content into `./apirest` directory
```
.
├── src
│   └── main
...
Then, access into the container to install and compile the sample application, and restart Supervisor
```bash
$ make apirest-ssh

/var/www $ mvn clean package -DskipTests # -DskipTests should be just for installation purpose
/var/www $ cp target/api-springboot-0.0.1-SNAPSHOT.jar target/app.jar
/var/www $ supervisorctl -c /etc/supervisor/supervisord.conf reload
```
...
The Supervisor Java service is located at `./platform/nginx-java-17/docker/config/supervisor/conf.d/java.conf`. This will execute the container `/var/www/target/app.jar` Java file
```bash
[program:java]
...
Now you can see the Java application running succesfully by visiting http://localhost:{port}/ on browser, but without the database connection established correctly, the sample home page will show a **Postgres** failure message.
...
The sample has this `./apirest/src/main/resources/application.properties` configuration to test running containers at a time, before setting the database connection parameters
```
# --- DataSource settings ---
...
# --- Thymeleaf --- Hibernate session stays open during the rendering of a web view for statements
spring.jpa.open-in-view=false

# --- Liquibase --- This whole block can be removed after platform installation
...
Try connection by accessing the appilication / rest api container to debug errors
```bash
$ make apirest-ssh
...
Update `./apirest/src/main/resources/application.properties` by deleting the whole Liquibase block or just setting to **true** `spring.liquibase.enabled`
```
# --- Liquibase --- This whole block can be removed after platform installation
...
```bash
$ git clone https://github.com/pabloripoll/docker-platform-nginx-java-17-maven-pgsql-16.4
$ cd docker-platform-nginx-java-17-maven-pgsql-16.4
```

Repository directories structure overview:
```
.
├── apirest (Spring, SpringBoot, etc.)
│   ├── src
│   ├── target
│   ├── pom.xml
│   └── ...
...
Once the container is up, Supervisor will run the sample API script. See `./platform/nginx-java-17/docker/config/supervisor/conf.d/java.conf`
```bash
[program:java]
command=java -jar /var/www/target/app.jar
stdout_logfile=/dev/stdout
...
> **Note**: If .jar name is other, remember to modify this file.
